...
export default class extends BaseSchema {
  ...
  public async up() {
    ...
    this.schema.createTable(this.tableName, (table) => {
      ...
      // Asegúrate de que la migración de chats corre ANTES que esta.
      table
        ...
        .onDelete('CASCADE') // If a chat is deleted, delete associated messages
        .notNullable(); // This foreign key cannot be null

      // ELIMINAMOS TODO EL BLOQUE RELACIONADO CON user_id, según tu indicación.
      /*
      ...
      table
        ...
        .onDelete('CASCADE') // If a user is deleted, delete associated messages
        .notNullable(); // This foreign key cannot be null
      */
      ...
      // table.index(['chat_id']);
      // ELIMINAMOS EL ÍNDICE RELACIONADO CON user_id si existiera y estuviera descomentado.
      // table.index(['user_id']);
    });
  }
  ...
  public async down() {
    ...
    // Si tu down() tuviera código para eliminar solo la FK de user_id o la columna user_id,
    // también deberías eliminarlo. Pero dropTableIfExists elimina todo.

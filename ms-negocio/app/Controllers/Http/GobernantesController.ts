...
export default class GobernantesController {
    ...
    public async create({ request, response }: HttpContextContract) {
        ...
        return response.status(201).send({
            ...
            name: user.name,
            email: user.email,
            periodoInit: gobernante.periodo_init,
            periodoEnd: gobernante.periodo_end,
        })
    }
    ...
    public async find({ request, params, response }: HttpContextContract) {
        ...
        try {
            if (params.id) {
                ...
                const gobernantes = await Gobernante.all()
    ...
    public async update({ params, request, response }: HttpContextContract) {
        try {
            ...
            gobernante.periodo_init = periodoInit
            gobernante.periodo_end = periodoEnd
    ...
    public async asignarDepartamento({ params, request, response }: HttpContextContract) {
        try {
            const gobernante = await Gobernante.findOrFail(params.id)
            const token = request.header('Authorization')?.replace('Bearer ', '')
            const { nombreDepartamento, fechaInicio, fechaFin } = request.only(['nombreDepartamento', 'fechaInicio', 'fechaFin'])
    ...
    public async asignarMunicipio({ params, request, response }: HttpContextContract) {
        try {
            const gobernante = await Gobernante.findOrFail(params.id)
            const token = request.header('Authorization')?.replace('Bearer ', '')
            const { nombreMunicipio, nombreDepartamento, fechaInicio, fechaFin } = request.only(['nombreMunicipio', 'nombreDepartamento', 'fechaInicio', 'fechaFin'])
